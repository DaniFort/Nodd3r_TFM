...
for letter in LETTERS:
    t0 = datetime.now()
    if not os.path.exists(os.path.join('dataset',letter)):
    ...
    for img_name in imgs:
        ...
        if hands:
            ...
            try:
                ...
                if has_hands:
                    counter+=1
                    category = 'train' if counter < 500 else 'val' if counter < 700 else 'test'
                    dst = os.path.join('dataset',letter,img_name)
                    cv2.imwrite(dst,img)
